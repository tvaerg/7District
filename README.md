...
1. **Python 3.10 eller senare** installerat. Kontrollera med:
   ```bash
   python --version
   ```
...
2. **Git** installerat, för att hämta koden. Kontrollera med:
   ```bash
   git --version
   ```
   Saknas det, ladda ner från https://git-scm.com/downloads.

3. **Två egna API-nycklar** (se avsnitt 4). Programmet anropar två AI-tjänster, en för att läsa PDF:er och en för analysen. Du måste sätta upp dina egna nycklar, de gamla nycklarna från utvecklingen ska inte användas i drift.

---

## 2. Hämta koden från GitHub

Öppna en terminal (Terminal på Mac, eller PowerShell/Command Prompt på Windows), gå till mappen där du vill ha projektet och kör:
...
Byt ut `<REPO_URL>` mot adressen till repot (den får du av Tom, den ser ut ungefär som `https://github.com/<anvandare>/<repo>.git`).

Om repot är privat kommer Git att be om inloggning. Använd ditt GitHub-konto. Behöver du åtkomst till repot, be Tom bjuda in dig som collaborator på GitHub.

Senare, när det kommit uppdateringar i koden, hämtar du dem med:
```bash
git pull
```
...
Skapa en ny textfil i projektmappens rot, döp den exakt till `.env` (med punkten först, inget annat), och klistra in följande, med dina egna nycklar inklistrade:
...
Det är allt som krävs. Spara filen.

Du kan skapa filen från terminalen om du vill:
...
├── input/                    <- lagg bolagens rapporter här (valfritt, se nedan)
...
`.env`-filen hittas inte eller saknar nyckeln. Kontrollera att filen heter exakt `.env`, ligger i projektmappens rot, och innehåller rätt rader utan mellanslag runt `=`.
